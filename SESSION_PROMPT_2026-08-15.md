...
#### 1. Генерация картинок через Runway — подключена
- Используется `RUNWAY_API_KEY`; `generate_image.js` должен запускаться с
  движком `runway` и теперь выбирает его по умолчанию
- Проверить на приёмке: одна согласованная генерация для обложки/сторис,
  корректная отправка результата и прозрачное сообщение при нехватке кредитов
